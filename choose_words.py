...
class EnglishArticleTool:
    def __init__(self, root):
        ...
        ttk.Button(button_frame, text="粘贴", command=self.paste_text).pack(side=tk.LEFT, padx=5)
    ...
    def paste_text(self):
        ...
        try:
            ...
            if result is None:  # 用户点击取消
                return
            elif result:  # 用户点击"是" - 替换
                self.content = clipboard_text
                self.text_widget.delete(1.0, tk.END)
                self.clear_selected()
                self.text_widget.insert(tk.END, self.content)
                self.current_file = None
                self.root.title("英文文章交互工具 - 粘贴的内容")
            else:  # 用户点击"否" - 追加
                # 添加一个换行符分隔原有内容和粘贴内容
                if self.content:
                    self.content += "\n" + clipboard_text
            ...
            messagebox.showinfo("成功", f"已成功粘贴文本（{len(clipboard_text)} 字符）")
            
        except tk.TclError:
            messagebox.showerror("错误", "无法访问剪贴板或剪贴板为空")
        except Exception as e:
